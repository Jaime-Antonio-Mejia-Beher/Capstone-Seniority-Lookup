...
async function fetchData() {
    try {
        const response = await fetch('http://localhost:4900/employees')
...
submitBtn.addEventListener("click", () => {
   ...
   const filterUsername = Employee.filter( item => {    // returns the array with only employee object that matches user input
    ...
    const totalEmp = filterEmpArray.length 
    const rankResult = `You are ${userRank} out of ${totalEmp} employees`;
    ranking.innerText = rankResult
    })
