...
const stationsData = ["Billings", "Scurry", "Fremont", "Roseburg", "Denver"];
const station = document.getElementById("station");

stationsData.forEach(item => {  // item is billings or the value of the first index, before moving on to the next index.value
    const option = document.createElement("option")  //create new option tag that we will append to select later
    option.setAttribute("value", item);             // Need to set the value attribute to the item
    const input = document.createTextNode(item);    // will need to add a text node/child to option tag
    option.appendChild(input);                      // append text node to option tag
    station.appendChild(option);                    // append the option tag (with textNode) to select tag
})
...
submitBtn.addEventListener("click", () => {
    ...
    const station = document.getElementById("station") // do we need this variable locally if already declared globally
    const input = document.getElementById("userInput")
   const filterEmp = Employee.filter(item => {      // store filter output
    if (item.address.city === station.value) {      // if the city matches the input city from user
        return true;
    } else {
        return false;
    }
